...
.PHONY: build release lint test coverage coverage-summary coverage-html tarpaulin \
	machete outdated fuzz fuzz-build geiger audit deny doc doc-open doc-clean ci run help \
...
help:
	@echo "Targets: build release lint test coverage coverage-summary coverage-html tarpaulin"
	@echo "  machete outdated fuzz geiger audit deny doc ci run"
...
coverage:
	mkdir -p coverage
	RUSTUP_TOOLCHAIN=stable $(CARGO) llvm-cov --locked --lcov \
...
coverage-html:
	mkdir -p coverage
	RUSTUP_TOOLCHAIN=stable $(CARGO) llvm-cov --locked --html \
		--ignore-filename-regex 'examples/|src/main\.rs|src/test_env\.rs' \
...
tarpaulin:
	...
	$(CARGO) tarpaulin --locked \
		--exclude-files 'examples/*' 'src/main.rs' 'src/test_env.rs' \
